/**
 * Claude Code OAuth wire compatibility for Pi (HIV-2211).
 *
 * Rebuilt from the MIT-licensed paoloanzn/pi-black compatibility technique,
 * pinned to its validated Claude Code 2.1.224 protocol. See LICENSE.pi-black.
 * No token, identity or request body is logged or persisted here.
 */
...
export const CLAUDE_CODE_PROTOCOL_VERSION = "2.1.224";
...
export async function claudeCodeVersionFingerprint(messages: Message[]): Promise<string> {
	const prompt = firstUserPrompt(messages);
	const selected = [4, 7, 20].map((index) => prompt[index] || "0").join("");
	const input = new TextEncoder().encode(
		`59cf53e54c78${selected}${CLAUDE_CODE_PROTOCOL_VERSION}`,
	);
	const digest = await crypto.subtle.digest("SHA-256", input);
	return bytesToHex(new Uint8Array(digest)).slice(0, 3);
}

export async function buildClaudeCodeBillingHeader(messages: Message[]): Promise<string> {
	const fingerprint = await claudeCodeVersionFingerprint(messages);
	return `x-anthropic-billing-header: cc_version=${CLAUDE_CODE_PROTOCOL_VERSION}.${fingerprint}; cc_entrypoint=${CLAUDE_CODE_ENTRYPOINT}; ${CCH_PLACEHOLDER};`;
}

export function parseClaudeCodeIdentity(value: unknown): ClaudeCodeIdentity | undefined {
	if (
		!isObject(value) ||
...
/**
 * Explicit env identity always wins. A Hive-launched session must not borrow
 * the workstation's Claude identity: the centrally assigned OAuth account may
 * belong to someone else. Identity metadata is optional, so omission is safer
 * than attaching the wrong account UUID to a valid token.
 */
export async function discoverClaudeCodeIdentity(
	...
	const fromEnvironment = parseClaudeCodeIdentity({
		userID: env.CLAUDE_CODE_DEVICE_ID,
		oauthAccount: { accountUuid: env.CLAUDE_CODE_ACCOUNT_UUID },
	});
	if (fromEnvironment) return fromEnvironment;
	if (env.HIVE_LAUNCH_ID) return undefined;

	const path = configPath ?? join(env.CLAUDE_CONFIG_DIR || homedir(), ".claude.json");
	try {
...
export async function transformClaudeCodePayload(
	...
	if (!isObject(payload)) {
		throw new Error("claude-oauth expected an Anthropic JSON request object");
	}
	const existingSystem = Array.isArray(payload.system) ? payload.system : [];
	const firstSystemText = isObject(existingSystem[0]) ? existingSystem[0].text : undefined;
	const secondSystemText = isObject(existingSystem[1]) ? existingSystem[1].text : undefined;
	...
	const transformed: JsonObject = {
		...payload,
		system: [
			{ type: "text", text: await buildClaudeCodeBillingHeader(context.messages) },
...
export function patchClaudeCodeCch(serializedBody: string): string {
	...
	try {
		...
		throw new Error("claude-oauth expected the Anthropic SDK to serialize a JSON object");
	}
	if (
		...
		throw new Error("claude-oauth request is missing the billing system block");
	}
	const billingText = body.system[0].text;
	if (!billingText.startsWith("x-anthropic-billing-header: ")) {
		throw new Error("claude-oauth request has an invalid billing block");
	}
	if (!billingText.includes(CCH_PLACEHOLDER)) {
		if (/; cch=[0-9a-f]{5};$/u.test(billingText)) return serializedBody;
		throw new Error("claude-oauth request has an invalid cch billing value");
	}
	if (typeof body.model !== "string" || !("max_tokens" in body)) {
		throw new Error("claude-oauth request is missing model or max_tokens");
	}
...
export function createClaudeCodeFetch(fetchImplementation: FetchFunction): FetchFunction {
	return async (input, init) => {
		...
		if (input instanceof Request) {
			const request = new Request(input, {
				headers,
				body: patchClaudeCodeCch(await input.clone().text()),
			});
			return fetchImplementation(request);
		}
		throw new Error("claude-oauth request body is not available for cch patching");
	};
}

export function claudeCodeHeaders(sessionId: string | undefined): ProviderHeaders {
	return {
		"user-agent": `claude-cli/${CLAUDE_CODE_PROTOCOL_VERSION} (external, ${CLAUDE_CODE_ENTRYPOINT})`,
		"x-app": "cli",
