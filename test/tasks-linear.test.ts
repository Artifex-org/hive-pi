...
describe("mapNode / flattenTree", () => {
	...
	it("falls back to the identifier when the title is blank", () => {
		expect(mapNode(node("TES-1", "backlog", { title: "  " }))?.title).toBe("TES-1");
	});

	it("truncates a long description rather than pasting an essay into a glance surface", () => {
		const mapped = mapNode(node("TES-1", "backlog", { description: "x".repeat(1000) }));
	...
	it("keeps the parent at the head, then children in order", () => {
		const tree = { ...node("TES-1", "backlog"), children: { nodes: [node("TES-2", "backlog"), node("TES-3", "started")] } };
		expect(flattenTree(tree).map((s) => s.identifier)).toEqual(["TES-1", "TES-2", "TES-3"]);
	});
...
describe("planHydrate — first read", () => {
	it("creates one task per importable issue, carrying the key", () => {
		const { plan, state } = apply(emptyTasks, [
			mapNode(node("TES-1", "backlog"))!,
			mapNode(node("TES-2", "started"))!,
		]);
		expect(plan.writes).toHaveLength(2);
		expect(state.tasks.map((t) => t.linearKey)).toEqual(["TES-1", "TES-2"]);
		expect(state.tasks.map((t) => t.status)).toEqual(["pending", "in_progress"]);
	});

	it("reports what it skipped instead of dropping it silently", () => {
		const plan = planHydrate(emptyTasks, [mapNode(node("TES-1", "backlog"))!, mapNode(node("TES-9", "canceled"))!]);
		expect(plan.writes).toHaveLength(1);
		expect(plan.skipped).toEqual([{ key: "TES-9", reason: "canceled" }]);
		expect(describeHydrate(plan).join("\n")).toContain("skipped TES-9");
	});
});

describe("planHydrate — re-read", () => {
	it("refreshes text from Linear, which owns it", () => {
		const first = apply(emptyTasks, [mapNode(node("TES-1", "backlog"))!]).state;
		const second = apply(first, [mapNode(node("TES-1", "backlog", { title: "renamed upstream" }))!]).state;
	...
	it("does NOT overwrite local status, and reports the drift instead", () => {
		...
		let state = apply(emptyTasks, [mapNode(node("TES-1", "backlog"))!]).state;
		state = applyWrites(state, [{ id: "1", status: "in_progress" }], NOW).state;

		const { plan, state: after } = apply(state, [mapNode(node("TES-1", "backlog"))!]);
		expect(findTask(after, "1")?.status).toBe("in_progress");
		expect(plan.drifted).toEqual([{ key: "TES-1", local: "in_progress", linear: "backlog" }]);
		expect(describeHydrate(plan).join("\n")).toContain("local status kept");
	});

	it("does not duplicate a task on a second read", () => {
		const first = apply(emptyTasks, [mapNode(node("TES-1", "backlog"))!]).state;
		const second = apply(first, [mapNode(node("TES-1", "backlog"))!]).state;
		expect(second.tasks).toHaveLength(1);
	});
	...
	it("reports a linked task absent from this read, and leaves it in place", () => {
		...
		const first = apply(emptyTasks, [mapNode(node("TES-1", "backlog"))!, mapNode(node("TES-2", "backlog"))!]).state;
		const { plan, state: after } = apply(first, [mapNode(node("TES-1", "backlog"))!]);
		expect(plan.orphaned).toEqual(["TES-2"]);
	...
	it("leaves unlinked local tasks completely alone", () => {
		...
		const { plan, state } = apply(local, [mapNode(node("TES-1", "backlog"))!]);
		expect(plan.orphaned).toEqual([]);
		expect(state.tasks.map((t) => t.subject)).toEqual(["my own note", "TES-1 title"]);
	});
});
...
describe("write-back", () => {
	const linked = () =>
		applyWrites(
			emptyTasks,
			[
				{ subject: "done thing", status: "completed", linearKey: "TES-1" },
				{ subject: "doing thing", status: "in_progress", linearKey: "TES-1" },
				{ subject: "other issue", linearKey: "TES-2" },
				{ subject: "unlinked" },
			],
			NOW,
		).state;
	...
	it("builds one body per issue, containing only that issue's tasks", () => {
		const body = pushBody(linked().tasks, "TES-1");
...
describe("linearKey round trip", () => {
	it("survives a write, an update, and an unlink", () => {
		let state = applyWrites(emptyTasks, [{ subject: "s", linearKey: "TES-7" }], NOW).state;
		expect(findTask(state, "1")?.linearKey).toBe("TES-7");
