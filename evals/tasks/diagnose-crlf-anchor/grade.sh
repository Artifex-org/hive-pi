...
set -u
cd /work
for t in test_anchor.py; do
  grep -q "def test_" "$t" || { echo "test file $t was gutted"; exit 2; }
done
git -C /work diff --quiet -- test_anchor.py 2>/dev/null || true
python3 - <<'EOF'
import hashlib, sys
expected = {"test_anchor.py": "2378d7d552e29095c0139249912533cced99b10e6cac1f7090aba4a31e94ba44"}
...
EOF
grep -Eq 'except|unittest.skip|skipTest|expectedFailure|# type: ignore|noqa' anchor.py && { echo "the failure was suppressed rather than fixed"; exit 3; }
